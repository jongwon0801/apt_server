...
angular.module('inspinia',
  ...
  .controller('HouseListCtrl', function($rootScope,$scope,$http,$location,$filter,$q,$stateParams,$state,$modal,MyCache) {
	...
	//$scope.sform.dong=$scope.sform.dong ||'';
	//$scope.sform.ho=$scope.sform.ho ||'';
	...
	$scope.pageChanged = function(p) {
        ...
        console.log(p);
		$scope.sform.page=p;
		$state.go('house.houselist',$scope.sform); 
    };	
	// 초기 데이타로드 
	$scope.doSearch=function(){
        console.log('dSearch');
        $http.get('/buyer/House',{
            params:  $scope.sform,
	...
	$scope.createHouse=function(){
        $http.post('/buyer/House/'+$stateParams.yid, $scope.eform).then(function(rs){
            console.log(rs);
