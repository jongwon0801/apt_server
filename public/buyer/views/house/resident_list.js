...
angular.module('inspinia',
  ...
  .controller('ResidentListCtrl', function($rootScope,$scope,$http,$location,$filter,$q,$stateParams,$state,$modal,MyCache) {
    ...
    $scope.sform.dong=$stateParams.dong
    $scope.sform.ho=$stateParams.ho
	//$scope.eform={};
	//페이지이동
	$scope.pageChanged = function(p) {
        ...
        console.log(p);
		$scope.sform.page=p;
		$state.go('house.residentlist',$scope.sform); 
    };	
	// 초기 데이타로드 
	$scope.doSearch=function(){
        console.log('dSearch');
        $http.get('/admin/Resident',{
            params:  $scope.sform,
	...
	$scope.createHouse=function(){
        $http.post('/admin/Resident/'+$stateParams.yid, $scope.eform).then(function(rs){
            console.log(rs);
	...
	$scope.formSubmit=function(){
		...
		$scope.sform.page='';
        $state.go('applebox.residentlist',$scope.sform);
        //$scope.createHouse();
