...
.controller('ShopEditCtrl', function($rootScope,$scope,$http,$location,$filter,$q,$stateParams,$timeout,$window,$modal,MyCache,Shop,Member,$resource,$state) {
  ...
  $scope.loadData=function(){
	  ...
	  if($stateParams.shopSq){
			$q.all([Shop.get($stateParams).$promise,MyCache.loadGCode(['shop.status','shop.kind'])]).then(function(results){
			...
			$q.all([MyCache.loadGCode(['shop.status','shop.kind'])]).then(function(results){
