...
angular.module('inspinia',
  ...
  .controller('ProductListCtrl', function($rootScope,$scope,$http,$location,$filter,$q,Product,$stateParams,$state,MyCache) {
	...
	$scope.pageChanged = function(p) {
		$scope.sform.page=p;
		$state.go('shop.productlist',$scope.sform); 
    };
	...
	$scope.doSearch=function(){
		...
		$q.all([Product.query($scope.sform).$promise,$scope.myCache.loadGCode(['product.status'])]).then(function(results){
	...
	*/
	...
	$scope.itemUpdate=function(item){
		Product.update(item,function(res){
