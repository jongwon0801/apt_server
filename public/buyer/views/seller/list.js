...
angular.module('inspinia',
  ...
  .controller('ListCtrl', function($rootScope,$scope,$http,$location,$filter,$q,Seller,$stateParams,$state,$modal) {
	...
	$scope.pageChanged = function(p) {
		$scope.sform.page=p;
		$state.go('seller.list',$scope.sform); 
    };	
	// 초기 데이타로드 
	$scope.doSearch=function(){
		
		$q.all([Seller.query($scope.sform).$promise,$scope.myCache.loadGCode(['seller.status'])]).then(function(results){
			  $scope.totalItems = results[0].recordsTotal;
			  $scope.list=results[0].data;
			  $scope.currentPage = $scope.sform.page;
			  
			  //console.log($scope.myCache.get(['seller.status']));
		});
	}
