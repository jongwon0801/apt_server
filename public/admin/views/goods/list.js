...
angular.module('inspinia',
  ...
  .controller('GoodsListCtrl', function($rootScope,$scope,$http,$location,$filter,$q,Goods,$stateParams,$state,MyCache) {
	...
	$scope.pageChanged = function(p) {
		$scope.sform.page=p;
		$state.go('goods.list',$scope.sform); 
    };
	...
	$scope.doSearch=function(){
		...
		$q.all([Goods.query($scope.sform).$promise,$scope.myCache.loadGCode(['goods.kind','goods.status','goods.cate'])]).then(function(results){
	...
	$scope.itemUpdate=function(item){
		Goods.update(item,function(res){
