...
.controller('AppleboxDetailCtrl', function($rootScope,$scope,$http,$location,$filter,$q,$stateParams,$timeout,$window,$modal,MyCache,Applebox,Buyer,Member,$resource,$state) {
  ...
  $scope.$watch(
    ...
    function( newValue, oldValue ){
        ...
        if(newValue){
            if ($scope.item){
                $scope.item.applebox.yid=$scope.yid;
            }
        }
  });
  $scope.$watch(
    ...
    function( newValue, oldValue ){
        ...
        if(newValue){
            if ($scope.item){
                 $scope.item.applebox.buyerSq=$scope.buyer.buyerSq;
            }
        }
  });
  $scope.$watch(
    ...
    function( newValue, oldValue ){
        ...
        if(newValue){
            console.log(newValue.originalObject.yid);
            loadApplebox(newValue.originalObject.yid);
        }
        
  });
  function loadApplebox(yid){
      //x`x`console.log(yid);
       if(!yid) return;
    $http.get('/v1/AppleboxAll/'+yid,null).then(function(results){
        $scope.item = results.data;
        if($scope.buyer) $scope.item.applebox.buyerSq=$scope.buyer.originalObject.buyerSq;
        if($scope.yid) $scope.item.applebox.yid=$scope.yid;
   ...
   $scope.formSubmit=function(){
       //if($scope.buyer) {
           try {
                ...
                $http.post('/v1/AppleboxCreate', content,{headers:{'Content-Type': 'application/json'}}).then(function(res){
