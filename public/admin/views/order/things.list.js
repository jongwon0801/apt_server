...
angular.module('inspinia',
  ...
  .controller('OrderThingsListCtrl', function($rootScope,$scope,$http,$location,$filter,$q,Things,$stateParams,$state,Shop) {
	...
	$scope.doSearch=function(){
		...
		$q.all([Things.query($scope.sform).$promise,Shop.query({kind:'A'}).$promise,$scope.myCache.loadGCode(['order.status.B'])]).then(function(results){
	...
	$scope.complete=function(item,status){
		...
		var req={status:status,shopSq:item.shopSq,thingsSq:item.thingsSq};
		//req.status=status;
