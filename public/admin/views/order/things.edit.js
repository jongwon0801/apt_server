...
.controller('OrderThingsEditCtrl', function($scope,$http,$state,$modal,$timeout,$q,Shop,$stateParams,MyCache,Things) {
...
if($stateParams.thingsSq){
    $q.all([Things.get($stateParams).$promise,Shop.query({kind:'A'}).$promise,$scope.myCache.loadGCode(['order.status.B','things.method'])]).then(function(results){
...
swal({
    ...
    $scope.trySave=function(){ // 처리 위탁 

//$scope.item.stepList = $scope.stepList;

            data = {shopSq:$scope.item.shopSq,thingsSq:$scope.item.thingsSq,status:$scope.item.status};
            Things.update(data,function(res){
                    toastr.success('수정하였습니다.');
                    $state.go('order.thingslist');
            },function(err){
                ERROR($state,err);
            });
    ...
    $scope.tryCancel=function(){ // 처리 위탁 

        //$scope.item.stepList = $scope.stepList;
        
                    data = {shopSq:$scope.item.shopSq,status:'Z',thingsSq:$scope.item.thingsSq};
                    Things.update(data,function(res){
                            toastr.success('수정하였습니다.');
                        $state.go('order.things.ist');
                    },function(err){
                        ERROR($state,err);
                    });
