...
angular.module('inspinia',
  ...
  .controller('MemberListCtrl', function($rootScope,$scope,$http,$location,$filter,$q,Member,$stateParams,$state,MyCache) {
	...
	$scope.pageChanged = function(p) {
		$scope.sform.page=p;
		$state.go('member.list',$scope.sform); 
    };
	...
	$scope.doSearch=function(){
		...
		$q.all([Member.query($scope.sform).$promise,$scope.myCache.loadGCode(['member.status','member.memberType','member.media'])]).then(function(results){
