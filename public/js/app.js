(function () {
    angular.module('inspinia', [
    ...
    ]).factory('MyCache', function($cacheFactory,$http,$q,$rootScope,$ocLazyLoad,$modal,$window) {

		

        $rootScope.onMemberView=function(item){
			var myWindow = $window.open("/shared/user/index.html?memberSq="+item.memberSq, item.memberSq, "width=800,height=600");
		};
		...
		$rootScope.onOrderView=function(item){
			//if(angular.isObject(item)){
				viewModal('/shared/controller/modal.order.view.js','/shared/views/modal.order.view.html','lg','ModalOrderViewCtrl',item);
		...
		$rootScope.onMxView=function(item){
			//if(angular.isObject(item)){
				viewModal('/shared/controller/modal.mx.view.js','/shared/views/modal.mx.view.html','lg','ModalMxViewCtrl',item);
		...
		$rootScope.onTraceView=function(item){
			//if(angular.isObject(item)){
				viewModal('/shared/controller/modal.trace.view.js','/shared/views/modal.trace.view.html','lg','ModalTraceViewCtrl',item);
		...
		$rootScope.onThingsView=function(item){
			//if(angular.isObject(item)){
				viewModal('/shared/controller/modal.things.view.js','/shared/views/modal.things.view.html','lg','ModalThingsViewCtrl',item);
			/*	
			}else{
				$http.get('/v1/Things/'+item).then(function(response) {
					//$scope.item=response.data.data;
					viewModal('/shared/views/modal.things.view.html','lg','ModalThingsViewCtrl',response.data.data);
				}, function(x) {
					console.log(x);
				});
			}*/
		...
		$rootScope.onShopView=function(item){
			$ocLazyLoad.load(['/shared/controller/modal.shop.view.js']).then(function(){
				$modal.open({
					animation:true,
		...
		$rootScope.onAppleboxView=function(item){
			$ocLazyLoad.load(['/shared/controller/modal.applebox.view.js']).then(function(){
				$modal.open({
					animation:true,
    	...
    	var factory={};
		factory.pcode=['pcode'];
		for(var i=0; i<9; i++){factory.pcode.push('pcode.0'+i);}
